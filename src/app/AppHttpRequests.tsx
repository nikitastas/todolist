...
import { Todolist } from "../fatures/todolists/api/todolistsApi.types"
import { DomainTask, GetTasksResponse, UpdateTaskModel } from "../fatures/todolists/api/tasksApi.types"
...
export const AppHttpRequests = () => {
  ...
  const changeTaskTitleHandler = (title: string, task: DomainTask) => {
    ...
    tasksApi.changeTaskTitle({ taskId: task.id, todolistId: task.todoListId, model }).then((res) => {
      const newTasks = tasks[task.todoListId].map((t) => (t.id === task.id ? { ...t, ...model } : t))
      setTasks({ ...tasks, [task.todoListId]: newTasks })
    })
  }
